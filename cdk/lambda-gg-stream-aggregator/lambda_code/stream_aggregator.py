...
def read_from_stream_aggregate_and_publish(client: StreamManagerClient):
    ...
    logger.info(len(raw_stream_data))

    result = []

    ids = map(lambda m: m["device_id"], raw_stream_data)
    distinct_ids = set(ids)
    for id in distinct_ids:
        values = [(v['voltage'], v['timestamp']) for v in raw_stream_data if v['device_id'] == id]
        ...
        result.append({'device_id': id, 'current_voltage': latest_v, 'time_left': timeleft})
